...
import com.zh.physiology.assist.PxUtils;
 ...
 * description：
 */
public class HeartRatePanelView extends View {

    private float mPercent; //seekBar传入的值
    ...
    //圆弧的填充色
    private int thickArcColor = Color.GREEN;
...
//    private Paint paintInerArc_tranform;//内圈蓝色画笔
    ...
    public HeartRatePanelView(Context context) {
        this(context, null);
        init(context);
    }

    public HeartRatePanelView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
        init(context);
    }

    public HeartRatePanelView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        drawOutAcr(canvas);//最外面线条
        drawerNum(canvas);//绘制刻度
        drawInArc(canvas, percent);//绘制粗圆弧
        drawInPoint(canvas);//绘制中间小圆和圆环
        drawerPointer(canvas, percent);//绘制指针
        drawerRecAndText(canvas, percent);//绘制矩形和文字
    }
    ...
    private void drawerNum(Canvas canvas) {
        ...
        for (int i = 0; i < mTikeCount + 1; i++) {
            ...
            canvas.drawText("" + (i*20), getWidth() / 2 - mArcWidth * 2, 40, paintouter_Num);//画刻度
            canvas.restore();
        }
        canvas.restore();
    ...
    private void drawInArc(Canvas canvas, float percent) {
        ...
        Paint paintInerArc_tranform = new Paint();
        ...
        paintInerArc_tranform.setColor(thickArcColor);
     ...
     * @param thickArcColor
     */
    public void setThickArcColor(int thickArcColor) {
        this.thickArcColor = thickArcColor;
    }
